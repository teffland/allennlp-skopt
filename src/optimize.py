...
def parse_args(args=[]):
    parser = ArgumentParser(description="Optimize allennlp model hyperparams with gaussian process")
    ...
    parser.add_argument('-m', '--mode', type=str, default='tree', help="Minimizer type. gp is gaussian process, random is random search.")
    parser.add_argument('--n-random-starts', type=int, default=0, help="If provided, seed process with n random function evals in addition to the defaul x0")
    parser.add_argument('--xi', type=float, default=0.01, help="Tree based")
    parser.add_argument('--kappa', type=float, default=1.96, help="Tree based")
    parser.add_argument('--no-delete-worse', action='store_true', help='By default we delete heavy files for worse trials as we go. This disables that.')
